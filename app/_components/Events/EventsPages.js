  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-2 flex flex-col gap-8 mb-[120px] overflow-hidden">
      ...
      <div className="w-full flex justify-between">
        <h2 className="text-3xl max-mdx:text-2xl font-semibold uppercase mt-[40px]">
          Мероприятия
        </h2>
        <button
          onClick={() => setAdminModal(true)}
          className="py-3 px-12 bg-redMain text-white font-semibold text-xl"
        >
          Добавить
        </button>
      </div>
        ...
        {cities.map((city) => (
          <button
            ...
            className={`px-4 py-2 rounded-full ${
              selectedCity === city ? "bg-[#E31E24] text-white" : "bg-gray-100 "
            }`}
          >
            {city}
          </button>
